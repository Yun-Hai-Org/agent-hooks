...
默认：JS 用 `bun`（`pnpm` 默认允许，`pnpm-install` 规则关闭）；Python 用 `uv`。默认仍拦截 npm / yarn / npx / pip / node。

规则均可在 quality-gate.yaml 配置：`ide.block-dangerous-commands.rules.<id>.enabled`。
常见 id：`pip-install`、`npm-install`、`npm-ci`、`pnpm-install`、`yarn-install`、`npx`、`python-script`、`python3-script`、`node-script`。
设 `enabled: false` 即放行该工具。全局 `~/.claude/quality-gate.yaml` 与仓库 `.claude/quality-gate.yaml` 合并；改 example 后的全局默认可能需再跑 `scripts/bootstrap-global-quality-gate.sh`。

| 操作            | ❌ 默认禁止                | ✅ 请使用              |
| --------------- | -------------------------- | ---------------------- |
| Python 依赖管理 | pip install / pip3 install | uv add                 |
| Python 脚本运行 | python / python3           | uv run python          |
| JS 依赖管理     | npm install / yarn         | bun add（pnpm 默认可） |
| JS 脚本运行     | node                       | bun                    |
| 包执行          | npx                        | bunx                   |
