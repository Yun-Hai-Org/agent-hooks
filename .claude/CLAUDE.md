...
- **工具限制**：默认 Python 用 `uv`，JS 优先 `bun`（`pnpm` 默认允许，`pnpm-install` 关闭）；npm / yarn / npx / pip / node 等仍默认拦截。均可在 quality-gate.yaml 用 `ide.block-dangerous-commands.rules.<id>.enabled` 开关（`enabled: false` 放行）。
...
| JS 依赖管理     | npm install / yarn add               | bun add（pnpm 默认允许）      |
