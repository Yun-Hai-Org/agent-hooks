...
const PATTERNS = [
  ...
  { level: 'high', id: 'git-push-main', regex: /\bgit\s+push\b(?!.*--force).*\b(origin\s+)?(main|master)\b/, reason: '禁止直接 push 到 main/master，请通过 PR/MR 合并' },
  // 22. git push --force-with-lease main/master
  { level: 'high', id: 'git-force-lease-main', regex: /\bgit\s+push\b.+--force-with-lease.*\b(main|master)\b/, reason: '禁止 force push 到 main/master' },
  ...
  { level: 'strict', id: 'npm-install', regex: /\bnpm\s+(install|i|uninstall)\b/, reason: '请使用 bun add / bun remove' },
  // 25. npm ci
  ...
  // 29. python <script>
  { level: 'strict', id: 'python-script', regex: /\bpython\s+[^-\s][^\s]*\.py\b/, reason: '请使用 uv run python <script.py>' },
  // 30. python3 <script>
  { level: 'strict', id: 'python3-script', regex: /\bpython3\s+[^-\s][^\s]*\.py\b/, reason: '请使用 uv run python <script.py>' },
  // 31. node <script>
  { level: 'strict', id: 'node-script', regex: /\bnode\s+[^-\s][^\s]*\.js\b/, reason: '请使用 bun <script.js>' },

  // ==================== STRICT - Hook 绕过防护 ====================
  // 32. git -c core.hooksPath=...
  { level: 'strict', id: 'hook-bypass-path', regex: /\bgit\b.*-c\s+core\.hooksPath=/, reason: '禁止通过 core.hooksPath 绕过 hook' },
  // 33. git commit --no-verify
  { level: 'strict', id: 'no-verify', regex: /\bgit\s+commit\b.*--no-verify/, reason: '禁止使用 --no-verify 跳过 hooks' },
  // 34. git commit -n
  { level: 'strict', id: 'no-verify-short', regex: /\bgit\s+commit\b.*\s-n\b/, reason: '禁止使用 -n(--no-verify) 跳过 commit hooks' },
...
function log(data) {
  try {
    ...
    appendFileSync(file, JSON.stringify({ ts: new Date().toISOString(), hook: 'block-dangerous-commands', ...data }) + '\n');
  } catch {}
}
