...
describe('resolve-hook-path', () => {
  ...
  describe('resolveHookPath', () => {
    it('项目级钩子优先于全局钩子', () => {
      ...
      writeFileSync(join(projectHooksDir, 'test-hook.ts'), '// project hook');

      const result = resolveHookPath('test-hook.ts', tempDir);
      expect(result).not.toBeNull();
      expect(result.source).toBe('project');
      expect(result.path).toBe(join(projectHooksDir, 'test-hook.ts'));
    });
    ...
    it('默认使用 process.cwd() 作为工作目录', () => {
      const result = resolveHookPath('security-orchestrator.ts', PROJECT_ROOT);
      expect(result).not.toBeNull();
      expect(result.source).toBe('project');
      expect(result.path).toContain('security-orchestrator.ts');
    });
