...
describe('pre-merge-commit integrator merge skip', () => {
  ...
  function writeSettings(integratorMergeRequiresFull: boolean): void {
    ...
    writeFileSync(
      ...
      ['settings:', '  worktree:', `    integratorMergeRequiresFull: ${integratorMergeRequiresFull}`].join('\n'),
      'utf-8',
    );
    clearGateConfigCache();
  }
  ...
  it('skips full gate for epic merging feat/*-task-* when integratorMergeRequiresFull is false', () => {
    writeSettings(false);
    expect(shouldSkipIntegratorMergeFullGate('feat/hooks-self-heal', 'feat/hooks-self-heal-task-1', tempDir)).toBe(
      true,
    );
  });
