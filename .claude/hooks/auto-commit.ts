...
export function buildFixFollowupMessage(
  gateResult: { results: { checkId: string; decision: string; message: string }[]; decision?: { reason?: string } },
 ...
 */
export async function runAutoCommit(cwd: string, options: { sessionId?: string } = {}) {
