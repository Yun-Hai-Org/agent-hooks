 ...
 *
 * 用法: bun .claude/hooks/resolve-hook-path.ts <hook-name>.ts
 *
 * 解析顺序:
 *   1. 项目级: <cwd>/.claude/hooks/<hook-name>.ts
 *   2. 全局级: ~/.claude/hooks/<hook-name>.ts
 *
 * 从 settings.json 调用示例:
 *   { "command": "bun .claude/hooks/resolve-hook-path.ts block-dangerous-commands.ts" }
 */
