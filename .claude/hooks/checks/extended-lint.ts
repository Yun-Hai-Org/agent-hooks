...
async function runExtendedChecks(
  ...
  if (classified.md.length > 0) {
    ...
    const runMdCheck = async (): Promise<CheckResult> => {
      try {
        const markdownlintTimeoutMs = Math.max(timeoutMs, MARKDOWNLINT_TIMEOUT_MS);
        const mdResult = await withTimeout(
          execCommandAsync(`${getBunxInvocation(cwd)} markdownlint-cli2 ${files}`, { cwd, timeout: markdownlintTimeoutMs }),
