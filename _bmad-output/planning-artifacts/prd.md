...
- 函数覆盖率: **84.84%**
- 行覆盖率: **64.97%**
- 测试用例数: **423 个**（从初始 115 个增加 268%）

**各文件覆盖率详情：**

| 文件 | 函数覆盖率 | 行覆盖率 | 状态 |
|------|-----------|---------|------|
| security-orchestrator.js | 100% | 99.31% | ✅ 完成 |
| block-dangerous-commands.js | 100% | 99.62% | ✅ 完成 |
| branch-gate.js | 100% | 70.94% | ✅ 函数完成，行待优化 |
| post-write-lint.js | 86.67% | 51.94% | 🟡 待优化 |
| protect-secrets.js | 80% | 90.89% | 🟡 待优化 |
| merge-gate.js | 70.37% | 48.75% | 🟡 待优化 |
| auto-stage.js | 75% | 38.36% | 🟡 待优化 |
| commit-gate.js | 66.67% | 19.92% | 🟡 待优化 |

**已完成的优化：**

1. **测试架构重构：** 从子进程测试改为直接函数调用，大幅提升测试效率和覆盖率
2. **main() 函数导出：** 所有钩子文件导出 main() 函数，支持直接测试
3. **stdin/stdout 模拟：** 使用 Readable 流和 console.log mock 测试完整流程
4. **边界条件覆盖：** 添加无效 JSON、空输入、错误处理等边界测试

**待优化的覆盖率缺口：**

1. **commit-gate.js (19.92% 行)：** 主流程分支逻辑、依赖检查、测试运行分支
2. **auto-stage.js (38.36% 行)：** Git 仓库检测、自动暂存逻辑
3. **merge-gate.js (48.75% 行)：** 安全扫描集成、分支检查逻辑
4. **post-write-lint.js (51.94% 行)：** 各 linter 的具体执行分支
5. **protect-secrets.js (90.89% 行)：** 接近完成，剩余为日志记录分支
